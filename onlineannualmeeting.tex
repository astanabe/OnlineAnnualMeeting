...
\item[Medium size: Max Height : ] 900
\item[Large size: Max Width : ] 1200
\item[Large size: Max Height : ] 1200
...
\item[Login Lockdown Options: Instantly Lockout Specific Usernames : ] 空欄
...
\item[Basic Firewall Rules: WordPress XMLRPC \& Pingback Vulnerability Protection: Completely Block Access To XMLRPC : ] チェックを入れる
\item[Basic Firewall Rules: WordPress XMLRPC \& Pingback Vulnerability Protection: Disable Pingback Functionality From XMLRPC : ] チェックを入れる
...
\subsection{LoginWPの設定}

左サイドバーの「LoginWP→Redirections」を以下のように設定します。
変更ごとに「Save Changes」を押して設定を保存する必要があります。

\begin{description}
\item[Redirection Rules : ] そのまま
\item[All Other Users: Login URL : ] \url{https://meeting2021.hogehoge.hoge/members/[variable]username[/variable]/profile/}
\item[All Other Users: Logout URL : ] \url{https://meeting2021.hogehoge.hoge/}
\item[After Registration: URL : ] \url{https://meeting2021.hogehoge.hoge/members/[variable]username[/variable]/profile/}
\item[Redirect Settings: Allow a POST or GET "redirect\_to" variable to take redirect precedence : ] No
\item[Redirect Settings: Allow a POST or GET "redirect\_to" logout variable to take redirect precedence : ] No
\end{description}
...
これに限らず、CSSの設定はKinstaのキャッシュ、WordPressのキャッシュ、ユーザーのブラウザのキャッシュによってすぐに反映して確認することができません。
そのため、まずMyKinstaにログインし、左サイドバーから「サイト→(Webサイト)→ツール」を開き、サイトキャッシュの「キャッシュをクリア」を押してサイトキャッシュを消し、「PHPの再起動」も押してPHPを再起動します。
CDNをもし有効化している場合、「サイト→(Webサイト)→Kinsta CDN」を開いて「CDNキャッシュをクリアする」ボタンを押します。
さらに、ユーザーのブラウザキャッシュを無効化するためにブラウザのプライベートブラウジング機能を使ってWebサイトにアクセスして下さい。
これでCSS編集結果が反映されると思います。
...
\item[Non-alphabetical affiliation : ] 例：保毛保毛大学
...
なお、\textbf{WordPress内でのユーザー名には、ハイフンとアンダースコア(アンダーバー)以外の記号は使わないで下さい}。
WordPressの仕様上はアットマークやピリオドも使えるのですが、BuddyPressの仕様上の都合でこれらの文字を使用すると不具合が発生します。
...
そこで、SendGridのマーケティングメール送信機能を使います。
この機能を使用すると、安全・迅速に多数のユーザーにメールを送信することができます。
マーケティングメールの送信機能の使用方法については、構造計画研究所の「\href{https://sendgrid.kke.co.jp/docs/Tutorials/B_Marketing_Mail/marketing_campaigns0.html}{マーケティングメール送信のチュートリアル}」をご覧下さい。

SendGridのマーケティングメールの送信では、送信先リスト(Contacts)を作成し、送信時に送信先リストを選択して送信することになります。
送信先リストを用意するために、まずWordPressからユーザー情報をexportします。
ユーザー情報のexportを行うには、ダッシュボードの左サイドバーから「Tools→Import and export users and customers」を開き、「Export」タブにある「Download」ボタンを押します。
これでCSV形式のユーザー情報リストがダウンロードできます。
CSV形式は表計算ソフトやテキストエディタで開くことができます。
ここで、「所属Group」が役に立ちます。
「所属Group」情報が「bp\_group\_id」カラムに登録されていますので、Groupごとにメンバーの行を取り出して別のCSV形式ファイルに保存します。
GroupごとのCSV形式ファイルの1行目はカラム名を示すラベル行にする必要がありますので、1行目は元のファイルと同じものにしておきます。
また、「bp\_group\_id」カラムの値はGroup名ではなくGroupごとに割り当てられた数値のリストです。
ダッシュボードの左サイドバーから「Groups→Groups」で作成済みGroupのリストを表示し、Group名のリンク先URLに含まれる「gid=数値」を確認することでGroupごとに割り当てられた数値がわかります。
また、GroupごとのCSV形式ファイルでは以下の3つのカラムを残して他のカラムは削除します。

\begin{description}
\item[user\_email : ] メールアドレス
\item[first\_name : ] 名
\item[last\_name : ] 姓
\end{description}

さらに、「user\_email」カラムの名前は「email」に変更します。
これでSendGridに送信先リストとしてアップロードできるようになりますので、Group名と同じ名前のリストとしてアップロードして下さい。

送信先リストが用意できたら、SendGridダッシュボードの「Marketing→Campaigns→Create a Campaign」からメール内容の作成・テストメール送信・本番のメール送信ができます。
...
変更後、SendGridのマーケティングメール送信機能を使用してtopicの作成権限を停止したことをAll membersグループのメンバー宛に送信して通知します。
また、Adjudicatorグループのメンバー宛に賞の審査員向けの投票ページがオープンになったことを通知します。
...
この機能は、\textbf{ユーザーごとに設定できるため、管理者アカウントでのみ有効化することも可能}です。
管理者アカウントでは有効化しておくことをおすすめします。
...
これで2要素認証の機能が加わります。
ただ、このプラグイン単独ではユーザーごとに有効化する必要があり、非常に煩雑です(管理者アカウントなど、数ユーザー程度であればそれで問題ありません)。
